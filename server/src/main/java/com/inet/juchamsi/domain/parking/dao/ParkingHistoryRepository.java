...
public interface ParkingHistoryRepository extends JpaRepository<ParkingHistory, Long> {
    ...
    @Query("select new com.inet.juchamsi.domain.parking.dto.service.ParkingHistoryDetailDto(u.loginId, pl.seatNumber, ph.outTime, u.carNumber, pl.frontNumber, pl.backNumber, ph.active, v.totalCount) from ParkingHistory ph left join ph.parkingLot pl left join ph.user u left join pl.villa v where v.idNumber=:villaIdNumber and u.loginId=:loginId and ph.createdDate in (select max(ph.createdDate) from ParkingHistory ph left join ph.user u where u.loginId=:loginId group by u.loginId)")
