...
public interface ChatPeopleRepository extends JpaRepository<ChatPeople, Long> {
    ...
    @Query("select cp from ChatPeople cp right join fetch cp.user u right join fetch cp.chatRoom cr where u.loginId=:loginId and cr.roomId=:roomId and u.active=:active and cr.status=:status")
